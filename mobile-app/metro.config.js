...
config.resolver = {
  ...
  resolveRequest: (context, moduleName, platform) => {
    ...
    if (moduleName.startsWith('react-native-safe-area-context')) {
      const redirected = moduleName
        .replace('react-native-safe-area-context/lib/module/', 'react-native-safe-area-context/src/')
        .replace('react-native-safe-area-context/lib/commonjs/', 'react-native-safe-area-context/src/');
      
      if (moduleName === 'react-native-safe-area-context') {
        return context.resolveRequest(context, 'react-native-safe-area-context/src/index', platform);
      }
      return context.resolveRequest(context, redirected, platform);
    }

    // Handle relative imports from within the library
    if (context.originModulePath && context.originModulePath.includes('react-native-safe-area-context')) {
  ...
  extraNodeModules: {
    ...
    'react-native-safe-area-context': path.resolve(__dirname, 'node_modules/react-native-safe-area-context/src'),
  },
