      ...
      >
        ...
        <View style={[styles.header, { backgroundColor: "#000" }]}>
          ...
          <View style={styles.neuralModule}>
            <View style={styles.moduleHdr}>
              ...
              <Text style={[styles.moduleVal, { color: colors.accent }]}>
                98.4%{t("ANALYTICS.HEALTH")}
              </Text>
            </View>
            <NeuralChart />
          </View>
              ...
              <View key={i} style={styles.distItem}>
                <View style={styles.distInfo}>
                  <Text style={[styles.distName, { color: colors.text }]}>
                    {t("ANALYTICS.SECTOR")}_0{i + 1}
                  </Text>
                  <Text style={[styles.distVal, { color: colors.primary }]}>
                    84%
                  </Text>
                </View>
                <View
                ...
                >
                  <View
                    style={[
                      styles.progressFill,
                      { width: "84%", backgroundColor: colors.primary },
                    ]}
                  />
                </View>
              </View>
        ...
        <View style={styles.diagGrid}>
          <DiagNode
            ...
            label={t("ANALYTICS.AI_NODE")}
            status={t("COMMON.OPTIMAL")}
            theme={theme}
          />
          <DiagNode
            icon={<Globe size={16} color={colors.accent} />}
            label={t("ANALYTICS.GRID_NODE")}
            status={t("COMMON.STATUS")}
            theme={theme}
          />
          <DiagNode
            icon={<Lock size={16} color={colors.warning} />}
            label={t("ANALYTICS.SEC_NODE")}
            status={t("DASHBOARD.HARDENED")}
            theme={theme}
          />
        </View>
      </ScrollView>
...
const MetricBlock = ({ label, value, sub, icon, theme }: any) => (
  ...
  >
    <View style={styles.metricHdr}>
      {icon}
      <Text style={[styles.metricLabel, { color: theme.colors.textLight }]}>
        {label}
      </Text>
    </View>
...
const DiagNode = ({ icon, label, status, theme }: any) => (
  ...
  >
    {icon}
    <Text style={[styles.diagLabel, { color: theme.colors.textLight }]}>
      {label}
    </Text>
    <Text style={[styles.diagStatus, { color: theme.colors.text }]}>
      {status}
    </Text>
  </View>
);
