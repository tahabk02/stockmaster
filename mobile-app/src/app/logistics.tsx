      ...
      >
        <View style={styles.cardHeader}>
          <View style={styles.idGroup}>
            ...
            >
              NODE_{item.trackingNumber?.substr(-6)}
            </Text>
          </View>
          <View
            style={[
          ...
          >
            <PulseIndicator
              color={isCritical ? theme.colors.danger : theme.colors.accent}
              size={4}
            />
            <Text
              style={[
        ...
        <View style={styles.routeContainer}>
          ...
          <View style={styles.routeLine}>
            ...
            />
            <Animated.View
              style={[styles.transitIcon, { left: `${progress}%` }]}
            >
              <Zap
                size={10}
                color={isCritical ? theme.colors.danger : theme.colors.primary}
                fill={isCritical ? theme.colors.danger : theme.colors.primary}
              />
            </Animated.View>
          </View>
          <View style={styles.routePoint}>
            ...
            >
              {item.destination?.city?.toUpperCase() || "UNKNOWN"}
            </Text>
          </View>
        </View>
        ...
        <View style={styles.metaRow}>
          <View style={styles.metaItem}>
            ...
            >
              ETA: <Text style={{ color: theme.colors.text }}>T+4H</Text>
            </Text>
          </View>
          <View style={styles.metaItem}>
...
export default function LogisticsScreen() {
    ...
    try {
      ...
      if (data.success) {
        ...
        setStats({
          ...
          transit: data.data.filter((d: any) => d.status === "IN_TRANSIT")
            .length,
          efficiency: "99.2%",
        });
        await sqliteService.logEvent(
          "LOGISTICS_SYNC",
          "SYSTEM",
          `Synced ${data.data.length} deliveries`,
          "INFO",
        );
      }
    } catch (e) {
  ...
  const updateStatus = async (id: string, status: string) => {
    try {
      ...
      const { data } = await api.put(`logistics/deliveries/${id}/status`, {
        status,
      });
      if (data.success) {
        fetchDeliveries();
      ...
      >
        <View style={styles.mapSection}>
          <View style={[styles.mapPlaceholder, { backgroundColor: "#020617" }]}>
            ...
            <View style={styles.mapTarget}>
              ...
              />
              <View
                style={[styles.targetCore, { backgroundColor: colors.primary }]}
              >
        ...
        <View style={styles.statsGrid}>
          <GlassCard style={styles.statBox} variant="dark">
            ...
            <Text style={[styles.statVal, { ...typography.pro }]}>
              {stats.active}
            </Text>
            <Text style={[styles.statLabel, { ...typography.pro }]}>
              ACTIVE_NODES
            </Text>
          </GlassCard>
          <GlassCard style={styles.statBox} variant="dark">
        ...
        <TouchableOpacity
          ...
          activeOpacity={0.8}
        >
          <Text style={[styles.dispatchText, { ...typography.pro }]}>
            INITIALIZE_NEW_DISPATCH
          </Text>
