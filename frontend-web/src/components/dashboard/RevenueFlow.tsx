...
export const RevenueFlow = ({ data, isRtl, t }: RevenueFlowProps) => (
  <div className="xl:col-span-2 bg-white/80 dark:bg-black/60 backdrop-blur-3xl p-8 rounded-[2.5rem] border border-slate-200/50 dark:border-white/10 shadow-[0_20px_60px_rgba(0,0,0,0.2)] relative group overflow-hidden h-full min-h-[380px]">
    ...
    <div className={cn("flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8", isRtl && "flex-row-reverse")}>
      <div className={cn("flex items-center gap-4", isRtl && "flex-row-reverse")}>
        ...
        <div className={isRtl ? "text-right" : "text-left"}>
          ...
          <p className="text-[9px] font-black text-slate-400 uppercase tracking-[0.3em] mt-2 italic">{t('dashboard.temporalResonance')}</p>
        </div>
      </div>
      <div className="flex gap-1.5 bg-slate-100 dark:bg-white/5 p-1.5 rounded-xl border border-slate-200 dark:border-white/10 shadow-inner">
         {['7D', '30D', 'ALL'].map(p => (
           <button key={p} className={cn("px-4 py-1.5 rounded-lg text-[8px] font-black transition-all border-none relative overflow-hidden", p === '30D' ? "bg-white dark:bg-indigo-600 text-indigo-600 dark:text-white shadow-lg" : "text-slate-400 hover:text-indigo-500")}>
