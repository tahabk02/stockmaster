...
export const RecentSignals = ({ orders, isRtl, t }: RecentSignalsProps) => (
  <div className="theme-card rounded-[2.5rem] overflow-hidden relative group">
    ...
    <div className={cn("p-6 md:p-8 border-b border-[var(--border)] flex justify-between items-center bg-[var(--bg)] dark:bg-white/[0.02] relative z-10", isRtl && "flex-row-reverse")}>
      <div className={cn("flex items-center gap-4", isRtl && "flex-row-reverse")}>
         <div className="p-3 bg-indigo-600 rounded-xl text-white shadow-xl">
            <Activity size={20} className="animate-pulse" />
         </div>
         <div>
            <h2 className="text-xl font-black text-slate-900 dark:text-white uppercase italic tracking-tighter leading-none">{t('dashboard.recentSignals')}</h2>
            <p className="text-[9px] font-black text-slate-600 dark:text-slate-500 uppercase tracking-[0.3em] mt-1.5 italic">LATTICE_STREAM</p>
         </div>
      </div>
        ...
        <tbody className="divide-y divide-[var(--border)] dark:divide-white/5">
          {orders.map((o: any, i) => (
            <tr key={i} className="group/row cursor-default">
              <td className="font-mono font-black text-[9px] text-slate-600 dark:text-slate-500 group-hover/row:text-indigo-500 transition-colors">#{o._id?.slice(-6).toUpperCase() || `ORD-0${i}`}</td>
              <td>
              ...
              <td className="text-right text-slate-600 dark:text-slate-500 font-black text-[9px] uppercase tracking-tighter italic">{new Date(o.createdAt).toLocaleDateString()}</td>
            </tr>
          ))}
        </tbody>
      </table>
